...
import com.medialog.common.ExcelManager;
...
public class BoardController {
	...
	public ModelAndView boardWrite(
		...
		UserVO board_user = userService.findOne((String)commandMap.get("ID"));
		logger.info(board_user.toString());

		commandMap.put("PWD",(String)board_user.getUser_pwd());
	...
	public @ResponseBody byte[] downloadExcelFile(
        ...
        String strToday = sdf2.format(c1.getTime());
        
		logger.info("downladExcelFile " + page_index +"~"+page_row);
...
/*		PagingRequest param = PagingRequest.create()
				.param("srchEventName", srchEventName);
*/		
		
		for(Map<String, Object> vo : list){
			List<Object> obj = new ArrayList<Object>();
			obj.add(vo.get("TITLE"));
			obj.add(vo.get("WRITER"));

			obj.add(vo.get("HIT_CNT"));
			obj.add(sdf.format(vo.get("CREA_DATE")));
			data.add(obj);
		}
		
		ExcelManager excelManager = new ExcelManager(header, data);
        excelManager.setSheetName("BoardList_"+strToday);
        excelManager.setWidth(6000);
        byte[] bytes = excelManager.makeExcel();

        response.setHeader("Content-Disposition", "attachment; filename=BoardList_"+strToday+".xlsx");
